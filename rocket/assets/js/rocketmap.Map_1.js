...
(function (window) {
    ...
    window.rocketmap.Map = function () {
        ...
        this.activatePolygonDrawning = function (drawendCallback) {
            ...
            if (this.draw === null || this.modify === null) {
                ...
                this.draw = new window.ol.interaction.Draw({
                    ...
                    //geometryFunction: window.ol.interaction.Draw.createRegularPolygon(4,0),
                    wrapX: true
                });
                //this.draw.createRegularPolygon(4,0);
                this.map.addInteraction(this.draw);
                ...
                this.draw.on('drawend',
                        function (evt) {
                            ...
                            if (geom instanceof window.ol.geom.Polygon) {
                                ...
                                //geom = this.correctWrapDateLine(geom);
                                //geom = this.moduloCoordinates(geom);
                                geom.transform('EPSG:4326', 'EPSG:3857');
                                evt.feature.setGeometry(geom);
                                this.featureOverlay.addFeature(evt.feature);
         ...
         */
        this.correctWrapDateLine = function (geometry) {
            ...
            for (var i = 0, ii = coordinates.length; i < ii; i++) {
                ...
                if (add360) {
                    for (var j = 0, jj = coordinates[i].length; j < jj; j++) {
                        coordinates[i][j] = [coordinates[i][j][0] + 360, coordinates[i][j][1]];
                    }
                }
            }
        ...
        this.moduloCoordinates = function (geometry) {
            ...
            for (var i = 0, ii = coordinates.length; i < ii; i++) {
                ...
                for (var j = 0, jj = coordinates[i].length; j < jj; j++) {
                    lon = coordinates[i][j][0];
                    if (lon > 180) {
                        while (coordinates[i][j][0] > 180){
                            coordinates[i][j][0] = coordinates[i][j][0] - 180;
                        }
                    } else if (lon <= -180) {
                        while (coordinates[i][j][0] < 180){
                            coordinates[i][j][0] = coordinates[i][j][0] + 180;
        ...
        this.constrainedCoordinates = function (geometry) {
